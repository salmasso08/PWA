...
let updateList = function(){
    ...
    Array.observe(notes.data, function(changes){
        ...
        if(status === 'removed'){
            let listOffNotes = document.querySelectorAll('#notes li');
            notesTag.removeChild(listOffNotes[index]);
        }
...
document.addEventListener('DOMContentLoaded', function(event){
    let listOffNotes = document.getElementById('notes');
    let listHtml = '';

    for (let i=0; i<notes.data.length; i++){
        listHtml += '<li>' + notes.data[i] + '</li>';
    }
    listOffNotes.innerHTML = listHtml;
